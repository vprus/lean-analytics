(function(root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        define(['exports', 'd3', 'dc.js'], function(exports, d3, dc) {
            factory(root, exports, d3, dc);
        });
    }
        ...
        // Browser global
        var UPC = {};
        factory(root, UPC, root.d3, root.dc);
        root.dc.unrolledPieChart = UPC.unrolledPieChart
    }

})(this, function(root, UPC, d3, dc) {

UPC.unrolledPieChart = function (parent, chartGroup) {
    ...
    function calculateAxisScale() {
        if (!_x || _elasticX) {
            var extent = d3.sum(_rowData, _chart.cappedValueAccessor);            
            _x = d3.scale.linear().domain([0, extent])
    ...
    function updateElements(rows) {
        ...
        var height;
        if (!_fixedBarHeight) height = (_chart.effectiveHeight() - _gap);
        ...
        for (i = 0; i < n; ++i) {
            ...
            if (start == undefined)
                start = _x(0) == -Infinity ? _x(1) : _x(0);            
            var width = Math.abs(_x(_chart.valueAccessor()(_rowData[i])));
            starts.push(start);
            widths.push(width);
            start += width;            
        }
